...
public class Sozcu {
    ...
    public void SozcuCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(19);
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element articleBodyDiv = detailDoc.selectFirst("div.article-body");

            if (articleBodyDiv != null) {
                return articleBodyDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
