...
public class ReelSektor {
    ...
    public void ReelSektorCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(14);
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element articleDiv = detailDoc.selectFirst("div.col-11.pl-3");

            if (articleDiv != null) {
                return articleDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
