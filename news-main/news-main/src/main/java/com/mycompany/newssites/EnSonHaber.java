...
public class EnSonHaber {

    public static void main(String[] args) {
        EnSonHaber e = new EnSonHaber();
        e.EnSonHaberCalis();
    }

    private static final Database database = new Database();

    public void EnSonHaberCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(5);
    ...
    private static void parseAndInsertLinks(String rssLink) {
        try {
            ...
            for (Element item : items) {
                ...
                // GUID etiketi içinden link alınıyor
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            // Belirtilen div'in içeriğini almak için
            Element articleDiv = detailDoc.selectFirst("div.column.column-article");
