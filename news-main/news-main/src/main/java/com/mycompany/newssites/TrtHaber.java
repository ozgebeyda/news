...
public class TrtHaber {
    ...
    public void TrtHaberCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(23);
    ...
    private static void parseAndInsertLinks(String rssLink) {
        try {
            ...
            for (Element item : items) {
                ...
                String photograph = item.selectFirst("imageUrl") != null
                        ? item.selectFirst("imageUrl").text()
                        : null;
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element newsContentContainerDiv = detailDoc.selectFirst("div.news-content-container");

            if (newsContentContainerDiv != null) {
                return newsContentContainerDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
