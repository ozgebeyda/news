...
public class SonDakika48 {

    public static void main(String[] args) {
        SonDakika48 s = new SonDakika48();
        s.SonDakika48Calis();
    }

    private static final Database database = new Database();

    public void SonDakika48Calis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(18);
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element newsDetailDiv = detailDoc.selectFirst("div.news-detail.px-4");

            if (newsDetailDiv != null) {
                return newsDetailDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
