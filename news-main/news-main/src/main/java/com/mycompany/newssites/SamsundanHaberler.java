...
public class SamsundanHaberler {

    public static void main(String[] args) {
        SamsundanHaberler s = new SamsundanHaberler();
        s.SamsundanHaberlerCalis();
    }

    private static final Database database = new Database();

    public void SamsundanHaberlerCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(17);
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element basliklarOnicerikDiv = detailDoc.selectFirst("div.basliklar_onicerik");

            if (basliklarOnicerikDiv != null) {
                return basliklarOnicerikDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
