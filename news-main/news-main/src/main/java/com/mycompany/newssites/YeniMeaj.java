...
public class YeniMeaj 
{

    public static void main(String[] args) 
    {
        YeniMeaj a = new YeniMeaj();
        a.YeniMesajCalis();
    }

     private static final Database database = new Database();

    public void YeniMesajCalis() 
    {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(25);
...
private static void parseAndInsertLinks(String rssLink) {
    try {
        ...
        for (Element item : items) {
            ...
            // <image> tagından fotoğraf URL'sini al
            Elements imageElements = item.select("image");
            if (!imageElements.isEmpty()) {
                photograph = imageElements.first().text(); // İlk <image> tagının metnini al
            } else {
                photograph = item.selectFirst("enclosure[type=image/jpeg]") != null
