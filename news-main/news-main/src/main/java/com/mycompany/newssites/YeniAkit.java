...
public class YeniAkit 
{

    public static void main(String[] args) 
    {
        YeniAkit a = new YeniAkit();
        a.YeniAkitCalis();
    }

     private static final Database database = new Database();

    public void YeniAkitCalis() 
    {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(24);
    ...
    private static void parseAndInsertLinks(String rssLink) {
        try {
            ...
            String siteName = doc.selectFirst("channel > copyright").text();
    ...
    try {
        ...
        Element defaultNewsDetailDiv = detailDoc.selectFirst("div.default-news-detail");
        
        if (defaultNewsDetailDiv != null) {
            return defaultNewsDetailDiv.outerHtml();  // Div'in dış HTML içeriğini al
        } else {
