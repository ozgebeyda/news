...
public class Haberis {

    public static void main(String[] args) {
        Haberis h = new Haberis();
        h.HaberisCalis();
    }

    private static final Database database = new Database();

    public void HaberisCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(7);
   ...
   private static String fetchArticleBody(String detailLink) {
    try {
        ...
        Element articleDiv = detailDoc.selectFirst("div.detay"); // 'div.detay' seçimi yapıldı
