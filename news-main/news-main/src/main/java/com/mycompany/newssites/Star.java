...
public class Star {
    ...
    public void StarCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(20);
    ...
    private static String fetchArticleBody(String detailLink) {
        try {
            ...
            Element newsWrapperDiv = detailDoc.selectFirst("div.news-wrapper");

            if (newsWrapperDiv != null) {
                return newsWrapperDiv.outerHtml();  // Div'in dış HTML içeriğini al
            } else {
