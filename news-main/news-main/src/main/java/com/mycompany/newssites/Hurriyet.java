...
public class Hurriyet {
    ...
    public static void main(String[] args) {
        Hurriyet a = new Hurriyet();
        a.HurriyetCalis();
    }

    private static final Database database = new Database();

    public void HurriyetCalis() {
        // Veritabanına bağlan
        database.connect();

        // site_id = 1 olan RSS linklerini al
        MongoCursor<Document> cursor = database.getRssLinks(9);
...
private static String fetchArticleBody(String detailLink) {
    try {
        ...
        Element articleDiv = detailDoc.selectFirst("div.news-content.readingTime"); // Burayı güncelledik
